...
namespace vractolib {
    ...
    void Drivetrain::moveToPoint(vunits::Pose pose, int timeout, int settleTime, int maxVolt, int feedforward) {
        ...
        while (elapsedTicks * vconfig::updateRate < timeout && settledTicks * vconfig::updateRate < settleTime) {
            ...
            const double targetAngle = std::atan2(pose.x - curPose.x, pose.y - curPose.y); //swapped becauuse we don't use standard unit circle for heading
            ...
            if (std::fabs(vunits::Pose::distance(curPose, pose)) < 2.5) {
                turnPIDOut = 0;
            }
